...
#adding foreign key to sales_fact to connect order_product_mapping
alter table sales_fact
add constraint
foreign key (order_id) references  order_product_mapping(order_id)

#adding foreign key to order_product_mapping to connect product_info
alter table order_product_mapping
add constraint
foreign key (product_id) references product_info(product_id)

# adding foreign key ‘sales_fact’  to connect it to the ‘customer_info’ table.
alter table sales_fact
add constraint customer_id_foreign_key
foreign key (customer_id) references customer_info(customer_id)

#adding foreign key to sales_fact to connect branch_details
alter table sales_fact
add constraint
foreign key (store_id) references branch_details(store_id);

#DML COMMANDS
# first insert values into dimention tabels 
# then insert values into mapping tables
# then insert values into fact tables


#insert values into  product_info
...
#insert values into branch_details
...
#insert values into  order_product_mapping
...
#1. the number of transactions involving male customers
select gender,count(order_id) 
from sales_fact
inner join customer_info
using(customer_id)
group by gender;

#2.the number of customers who placed an order from more than one store.
select 
   count(customer_id)
from
(select customer_id,COUNT(DISTINCT store_id) as store_count from sales_fact 
group by customer_id
having  count(distinct store_id)>1) as a;


#3the city with the highest number of orders, along with the number of orders.
select city ,count(order_id) as highest_order from sales_fact
inner join branch_details
using(store_id)
group by city 
order by count(order_id) desc ;

#4the area that recorded the highest sales in terms of sales amount, 
#along with its sales amount.
select area ,sum(sale_price * quantity_ordered) from sales_fact
inner join branch_details
using(store_id)
inner join order_product_mapping
using(order_id)
inner join product_info
using(product_id)
group by area
order by sum(sale_price * quantity_ordered) desc;


#find the state with the highest number of customers.
select state ,count(DISTINCT customer_id) from sales_fact
inner join branch_details 
using(store_id)
group by state
